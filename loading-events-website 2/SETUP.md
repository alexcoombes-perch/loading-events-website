...
2. Select the `loading-events-website` repo and authorize Cloudflare's GitHub integration.
3. On the build settings screen, set:
   - **Build command:** `npm install && npx eleventy`
   - **Build output directory:** `dist`
4. Click **Save and Deploy**. First build takes a minute or two — you'll get a `*.pages.dev` URL to preview it.
